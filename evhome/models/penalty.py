...
class PenaltyStatus(models.Model):
    id = models.AutoField(primary_key=True)
    penalty = models.ForeignKey(Penalties, on_delete=models.CASCADE, db_column='penalty_id')
    penalty_status_type = models.ForeignKey(PenaltyStatusTypes, on_delete=models.CASCADE, db_column='penalty_status_type_id')
    timestamp = models.DateTimeField()
